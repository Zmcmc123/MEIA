...
`codex/large-system-optimization` 保持与 `main` 隔离，待项目所有者用真实大 slab + 水体系验收后再决定是否合并。当前已完成：

- 根据实际可见周期实例选择自动/按需 2D 预览，并防止过期图像被当作当前产物下载。
- 紧凑的背景强度 + 主体例外、大体系 200 原子/页选择、稀疏 3D 选择层和 36 px 网格命中索引。
- 每个缩放帧一次 Plotly 批量更新，80 ms 最终同步；20,000 实例起在交互期临时简化键描边和氢键图层。
- 会话内单条 `RenderTopology` 缓存；仅颜色、强度、半径、键宽、相机或导出参数变化时不重新运行邻居列表和周期展开。
- 公开大体系案例从 `examples/CONTCAR` 经 `5×5×1` 扩胞并新增 200 个 H₂O 得到，共 6,225 个源原子；配套快照使用 `1×1×1` 周期范围，实际显示 6,225 个原子实例。

案例生成器固定校验源 CONTCAR 的 SHA-256 `187ee6a6d1c5bffc2b55a8ea254f0dc86c82a1f56743fcdad55504488b399d5f`。扩胞后晶胞长度为 `67.825385 × 51.101150 × 32.732590 Å`，新增水氧原子位于 `z = 25.2/28.6 Å` 两层。ASE 周期近邻审计中，新增水与原体系最近非成键接触为 3.027 Å，不同新增水分子间最近接触为 3.196 Å；没有低于 `0.95 ×` 元素对共价半径和的异常接触。
...
可通过 `python scripts/benchmark_large_system.py --help` 调整 slab 尺寸、水层数和 a/b/c 周期范围。该基准的 2D 路径和最终导出都使用完整数据。
