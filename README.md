...
- 三维旋转、平移、缩放和原子选择，并可将当前视角应用到二维投影。
...
公开合成案例包含 [6,246 原子的 slab + 水体系](examples/MEIA_large_slab_water_6246.xyz)
和对应的 [工作状态快照](examples/MEIA_large_slab_water_6246.workspace.meia.json)。
直接导入结构可测试大体系侧栏；导入快照并确认覆盖后，会以 `2×2×1`
显示 24,984 个原子实例，并将第一个水分子作为主体。该案例仅用于性能和交互测试，不代表经过验证的物理模型。
